...
@dataclass
class PositionState:
 status:PositionStatus=PositionStatus.FLAT;direction:str="flat";framework:str="";setup_id:str|None=None;entry_signal_timestamp:pd.Timestamp|None=None;activation_timestamp:pd.Timestamp|None=None;exit_request_timestamp:pd.Timestamp|None=None;bars_in_state:int=0;bars_since_entry_signal:int=0;stop_proposal:dict=field(default_factory=dict);target_proposal:dict=field(default_factory=dict);trailing_proposal:dict=field(default_factory=dict);maximum_holding_period:int|None=None;entry_reason:str="";latest_exit_reason:str="";transition_reason:str="initial";warnings:list[str]=field(default_factory=list);metadata:dict=field(default_factory=dict)
 def to_dict(self):
  d=asdict(self);d["status"]=self.status.value
  for k in ("entry_signal_timestamp","activation_timestamp","exit_request_timestamp"):
   d[k]=None if d[k] is None else pd.Timestamp(d[k]).isoformat()
  return d
@dataclass
class SetupState:
 status:SetupStatus=SetupStatus.NONE;setup_id:str|None=None;framework:str="";direction:str="flat";setup_type:str="";created_timestamp:pd.Timestamp|None=None;armed_timestamp:pd.Timestamp|None=None;trigger_timestamp:pd.Timestamp|None=None;expiration_timestamp:pd.Timestamp|None=None;source_level:float|None=None;bars_alive:int=0;maximum_bars_alive:int=5;trigger_count:int=0;metadata:dict=field(default_factory=dict);reason:str=""
 def to_dict(self):
  d=asdict(self);d["status"]=self.status.value
