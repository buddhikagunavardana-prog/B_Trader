...
from src.tests.historical_campaign_test_data import all_50_bindings
from src.trading_frameworks.registry import trading_framework_registry


def test_all_50_registry_configurations_plan_without_fixed_campaign_count(tmp_path):
    config = load_campaign_configuration("src/config/framework_research/historical/campaigns/structural_all_50.json")
    config = replace(config, output_root=str(tmp_path))
    bindings = all_50_bindings(20)
