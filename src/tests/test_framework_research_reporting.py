...
def test_inventory_validation_and_performance_reports_are_structural_only():
    ...
    assert len(inventory) == 7 and set(inventory["Status"]) == {"Complete"}
    assert len(validation) == 5 and set(validation["Result"]) == {"Pass"}
    assert len(performance) == 5 and (performance["Repeated Indicator Calculations"] == 0).all()
